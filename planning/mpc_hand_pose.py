...
class MPCHandPose:
    """
    MPC for hand pose matching - extends the explicit MPC to support
    hand pose targets (palm pose + finger joints) instead of object pose.
    """
    ...
    def plan_once(self, target_palm_pos, target_palm_quat, target_finger_qpos,
                  curr_x, phi_vec, jac_mat, sol_guess=None):
        """
        Plan one step for hand pose matching.
        ...
        # Pack cost parameters: target hand pose + contact info
        cost_params = cs.vvcat([target_palm_pos, target_palm_quat, target_finger_qpos,
                               phi_vec, jac_mat])
