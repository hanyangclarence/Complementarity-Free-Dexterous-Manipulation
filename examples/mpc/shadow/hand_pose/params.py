...
class HandPoseMPCParams:
    def __init__(self, rand_seed=1):
        ...
        # ---------------------------------------------------------------------------------------------
        self.model_path_ = 'envs/xmls/env_shadow_hand_pose.xml'
        ...
        target_palm_rpy = np.array([
            1.0 * (2 * np.random.rand() - 1),   # roll: ±1.0 rad
            1.0 * (2 * np.random.rand() - 1),   # pitch: ±1.0 rad
            1.0 * (2 * np.random.rand() - 1)    # yaw: ±1.0 rad
        ])
        ...
        finger_variation = (2 * np.random.rand(22) - 1) * 0.5
        self.target_robot_qpos_ = self.init_robot_qpos_ + finger_variation

        # Clip to actual joint limits from Shadow hand URDF/XML
        self.target_robot_qpos_ = np.clip(self.target_robot_qpos_,
                                          joint_limits_lower,
                                          joint_limits_upper)
        ...
        finger_u_bound = 0.05  # ±0.2 rad position delta per step for fingers
    ...
    def init_cost_fns(self):
        ...
        finger_qpos = x[14:36]   # all 22 finger joints
        ...
        final_finger_pos_weight = 20.0    # Strong final finger joint matching
