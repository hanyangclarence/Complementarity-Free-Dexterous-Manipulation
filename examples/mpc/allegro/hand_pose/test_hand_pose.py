...
from examples.mpc.allegro.hand_pose.params import HandPoseMPCParams
...
save_flag = False
if save_flag:
    save_dir = './examples/mpc/allegro/hand_pose/save/'
...
max_rollout_length = 500  # Longer horizon for hand pose matching
...
while trial_count < trial_num:
    ...
    print(f"  Target palm pos (qpos/local): {param.target_palm_qpos_[0:3]}")
    print(f"  Target palm quat: {param.target_palm_qpos_[3:7]}")

    # Compute what world position should be (corrected transform)
    target_world = np.array([
        param.target_palm_qpos_[2],   # world_x = qpos_z
        -param.target_palm_qpos_[1],  # world_y = -qpos_y
        param.target_palm_qpos_[0]    # world_z = qpos_x
    ])
    print(f"  Target palm pos (world, computed): {target_world}")
        ...
        if not env.dyn_paused_:
            ...
            if rollout_step % 10 == 0:
                ...
                if n_actual_contacts > 0:
                    # jac_mat is (max_ncon*4, n_qvel) = (80, 28)
                    # Columns: obj_vel(0:6), palm_vel(6:12), finger_vel(12:28)
                    obj_jac_norm = np.linalg.norm(jac_mat[:, 0:6])
                    palm_jac_norm = np.linalg.norm(jac_mat[:, 6:12])
                    finger_jac_norm = np.linalg.norm(jac_mat[:, 12:28])
            ...
            if rollout_step % 10 == 0:
                # Get actual palm physical position from MuJoCo
                actual_palm_xpos = env.data_.xpos[env.model_.body('palm').id]
                actual_palm_xquat = env.data_.xquat[env.model_.body('palm').id]

                # Compute current qpos -> world transform to verify (corrected)
                curr_qpos_to_world = np.array([
                    curr_palm_pos[2],   # world_x = qpos_z
                    -curr_palm_pos[1],  # world_y = -qpos_y
                    curr_palm_pos[0]    # world_z = qpos_x
                ])

                print(f"\n{'='*70}")
                print(f"Step {rollout_step}:")
                print(f"  Palm Position Error: {palm_pos_error:.6f} m")
                print(f"    Current qpos (from get_state): {curr_palm_pos}")
                print(f"    Current xpos (actual world):   {actual_palm_xpos}")
                print(f"    Current qpos->world (my calc): {curr_qpos_to_world}")
                print(f"    Target qpos:                   {param.target_palm_qpos_[0:3]}")
                print(f"  Palm Quaternion Error: {palm_quat_error:.6f}")
                print(f"    Current (from get_state): {curr_palm_quat}")
                print(f"    Current (from xquat):     {actual_palm_xquat}")
                print(f"    Target:                   {param.target_palm_qpos_[3:7]}")
                print(f"  Finger Joint Error: {finger_error:.6f} rad")
                print(f"    Current (first 4): {curr_finger_qpos[0:4]}")
                print(f"    Target (first 4):  {param.target_robot_qpos_[0:4]}")
